...
public class StudentCoursePlan {
	...
	public StudentCoursePlan(Student st, Curriculum cur, int maxCredits)
	{
		...
		_periodCredits = new int[cur.getMandatories().keySet().size()+1];
	}
	...
	public void add(Class c, boolean par)
	{
		...
		while(_periodCredits[period] + c.getWeight() > _maxCredits)
			period++;
		
		_periodCredits[period] += c.getWeight();
		...
		if(_periodCredits[_currentPeriod] == 20)
			_currentPeriod++;
	}
